...
<div x-data="{ actionUrl: '' }">
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <a href="{{ route('admin.collections.create') }}" class="bg-amber-900 text-white hover:bg-amber-800 uppercase tracking-widest text-xs font-bold rounded-xl px-8 py-4 transition-all duration-200 shadow-sm hover:shadow-md font-sans whitespace-nowrap">
            Nueva Colección
        </a>
    </div>
    ...
    <x-admin.table :headers="[
        ...
        @forelse($collections as $collection)
            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                <td class="px-6 py-4 font-sans font-semibold text-gray-900 text-sm">
                    {{ $collection->name }}
                </td>
                <td class="px-6 py-4 font-sans text-sm text-gray-500 font-medium">
                ...
                <td class="px-6 py-4 flex items-center gap-1 font-sans text-sm">
                    <a href="{{ route('admin.collections.edit', $collection) }}" class="p-2 text-gray-400 hover:text-amber-900 hover:bg-amber-50 rounded-xl transition-colors" title="Editar">
                    ...
                    <button @click="actionUrl = '{{ route('admin.collections.destroy', $collection) }}'; $dispatch('open-modal', 'delete-modal')" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors" title="Eliminar">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
        ...
        @empty
            <tr>
                <td colspan="4" class="px-6 py-16 text-center bg-white">
                    ...
                    <span class="text-base font-medium text-gray-400 font-sans">No se encontraron colecciones registradas.</span>
                </td>
            </tr>
        @endforelse
    </x-admin.table>
    ...
    <x-admin.modal id="delete-modal" title="Eliminar Colección">
        <p class="text-sm text-gray-600 leading-relaxed font-sans">¿Estás seguro de que deseas eliminar esta colección? Los productos que contiene <span class="font-semibold text-gray-900">no se eliminarán</span>, únicamente se desvincularán de esta agrupación.</p>
        <x-slot name="footer">
