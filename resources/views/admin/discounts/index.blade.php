    ...
    <div x-data="{ actionUrl: '' }">
        ...
        <x-admin.table :headers="[
            ...
            @forelse($discounts as $discount)
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 font-sans">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-600 font-sans capitalize">
                        @php
                            $appliesLabels = [
                                'all' => 'Toda la tienda',
                        ...
                        @if ($discount->starts_at || $discount->ends_at)
                            <div class="flex flex-col gap-1.5">
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] font-bold text-gray-400 tracking-widest uppercase w-10">Inicio</span>
                                    <span class="text-sm font-medium text-gray-700">{{ $discount->starts_at?->format('d/m/Y') ?? 'Siempre' }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] font-bold text-gray-400 tracking-widest uppercase w-10">Fin</span>
                                    <span class="text-sm font-medium text-gray-700">{{ $discount->ends_at?->format('d/m/Y') ?? 'Indefinido' }}</span>
                                </div>
                            </div>
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center gap-1 font-sans">
                        ...
                        <button @click="actionUrl = '{{ route('admin.discounts.destroy', $discount) }}'; $dispatch('open-modal', 'delete-modal')"
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-6 py-16 text-center bg-white">
                        ...
                        <span class="text-base font-medium text-gray-400 font-sans">No se encontraron descuentos registrados.</span>
                    </td>
                </tr>
            @endforelse
        </x-admin.table>
        ...
        <x-admin.modal id="delete-modal" title="Eliminar Descuento">
            <p class="text-sm text-gray-600 leading-relaxed font-sans">
                ¿Estás seguro de que deseas eliminar este descuento? Si ya tiene cupones asociados <span class="font-semibold text-gray-900">no podrá ser borrado</span> por integridad de los datos.
            </p>
            <x-slot name="footer">
