...
@section('header')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 font-sans mt-1">Realizado el {{ $order->created_at->format('d de F, Y \a \l\a\s H:i') }}</p>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100 text-sm font-sans">
                    <thead class="bg-white">
                        <tr>
                            <th class="px-8 py-4 text-left font-semibold text-gray-500 uppercase tracking-wider text-xs">Producto</th>
                            <th class="px-8 py-4 text-center font-semibold text-gray-500 uppercase tracking-wider text-xs">Cant.</th>
                            <th class="px-8 py-4 text-right font-semibold text-gray-500 uppercase tracking-wider text-xs">Precio Unitario</th>
                            <th class="px-8 py-4 text-right font-semibold text-gray-500 uppercase tracking-wider text-xs">Total</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @foreach($order->items as $item)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-8 py-4 font-medium text-gray-900">{{ $item->product->name }}</td>
                                <td class="px-8 py-4 text-center text-gray-600">{{ $item->quantity }}</td>
                                <td class="px-8 py-4 text-right text-gray-600">${{ number_format($item->unit_price, 2) }}</td>
                                <td class="px-8 py-4 text-right font-semibold text-gray-900">${{ number_format($item->quantity * $item->unit_price, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <h2 class="text-lg font-medium text-gray-900 font-serif mb-6 border-b border-gray-100 pb-4 flex items-center gap-2">
                ...
                @forelse($order->statusHistory as $hist)
                    <div class="relative pl-8">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-1">
                            ...
                            <time class="text-xs text-gray-500 font-sans mt-1 sm:mt-0">{{ $hist->changed_at->format('d/m/Y \a \l\a\s H:i') }}</time>
                        </div>
                        
                        <p class="text-sm text-gray-500 font-sans mt-2">
                            @if($hist->user) Modificado por <span class="font-medium text-gray-700">{{ $hist->user->first_name }}</span>. @endif
                            @if($hist->comment) <span class="italic block mt-1 bg-gray-50 p-2 rounded-lg border border-gray-100">"{{ $hist->comment }}"</span> @endif
                        </p>
                    </div>
                @empty
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900 font-sans">{{ $order->customer->user?->email ?? 'No disponible' }}</p>
                </div>
                @if($order->quotation)
                    <div class="pt-4 border-t border-gray-100 mt-4">
                        <p class="text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Origen</p>
                        <a href="{{ route('admin.quotations.show', $order->quotation) }}" class="inline-flex items-center gap-1 text-sm text-amber-700 hover:text-amber-900 font-medium transition-colors">
                            Ver Cotización Original
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"></path></svg>
                        </a>
                    </div>
                @endif
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="space-y-4">
                ...
                <select name="status_id" class="block w-full rounded-xl border-gray-200 py-3 text-sm focus:ring-amber-800 focus:border-amber-800 transition-colors shadow-sm font-sans bg-gray-50">
                ...
                <button type="submit" class="w-full justify-center inline-flex items-center {{ $order->status_id === \App\Enums\OrderStatus::CANCELLED ? 'bg-red-800 hover:bg-red-900' : 'bg-amber-900 hover:bg-amber-800' }} text-white uppercase tracking-widest text-xs font-bold rounded-xl px-6 py-3.5 transition-colors duration-200 shadow-sm font-sans">
                    Actualizar Estado
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin.orders.update-shipment', $order) }}" class="space-y-3">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Método</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Paquetería</label>
                        <input type="text" name="carrier" value="{{ $order->shipment->carrier ?? '' }}" placeholder="Ej. FedEx" 
                               class="block w-full rounded-xl border-gray-200 py-3 text-sm focus:ring-amber-800 focus:border-amber-800 bg-gray-50">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Costo Real</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Guía (Tracking)</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1">Entrega Estimada</label>
                    <input type="date" name="estimated_delivery_date" value="{{ optional($order->shipment)->estimated_delivery_date?->format('Y-m-d') }}" 
                           class="block w-full rounded-xl border-gray-200 py-3 text-sm focus:ring-amber-800 focus:border-amber-800 bg-gray-50">
                </div>

                <button class="w-full mt-2 justify-center inline-flex items-center bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 uppercase tracking-widest text-xs font-bold rounded-xl px-6 py-3.5 transition-colors shadow-sm font-sans">
                    Guardar Logística
                </button>
            </form>
        </div>
                ...
                @forelse($order->payments as $payment)
                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 relative">
                        <div class="flex justify-between items-start mb-2">
                            <span class="font-semibold text-gray-900 font-sans">${{ number_format($payment->amount, 2) }}</span>
                            <x-admin.badge :color="$payment->status_id->color()" :label="$payment->status_id->label()" />
                        </div>
                        
                        @if($payment->mp_transaction_id)
                            <p class="text-xs text-gray-500 font-mono break-all mb-2">Ref: {{ $payment->mp_transaction_id }}</p>
                        @endif
                        
                        @if($payment->status_id === \App\Enums\PaymentStatus::PENDING)
                            <form method="POST" action="{{ route('admin.orders.approve-payment', [$order, $payment]) }}" class="mt-3 border-t border-gray-200 pt-3">
                                @csrf @method('PUT')
                                <button class="w-full inline-flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-green-700 hover:text-green-800 transition-colors">
