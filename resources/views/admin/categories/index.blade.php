...
<div x-data="{ actionUrl: '' }">
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <a href="{{ route('admin.categories.create') }}" class="bg-amber-900 text-white hover:bg-amber-800 uppercase tracking-widest text-sm font-bold rounded-xl px-8 py-4 transition-colors duration-200 shadow-sm font-sans whitespace-nowrap">
            Nueva Categoría
        </a>
    </div>
    ...
    <x-admin.table :headers="[
        ...
        @forelse($categories as $category)
            <tr class="hover:bg-gray-50/50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 font-sans">{{ $category->name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">
                    ...
                    @else
                        <span class="text-gray-400 italic">Principal</span>
                    @endif
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">{{ $category->sort_order }}</td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium {{ $category->is_active ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-gray-100 text-gray-600 border border-gray-200' }}">
                        {{ $category->is_active ? 'Activa' : 'Inactiva' }}
                    </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center gap-2">
                    <a href="{{ route('admin.categories.edit', $category) }}" class="p-2 text-gray-400 hover:text-amber-900 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
                    ...
                    <button @click="actionUrl = '{{ route('admin.categories.destroy', $category) }}'; $dispatch('open-modal', 'delete-modal')" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
        ...
        @empty
            <tr>
                <td colspan="5" class="px-6 py-12 text-center">
                    ...
                    <span class="text-sm font-medium text-gray-500 font-sans">No se encontraron categorías.</span>
                </td>
            </tr>
        @endforelse
    </x-admin.table>
    ...
    <x-admin.modal id="delete-modal" title="Eliminar Categoría">
        <p class="text-gray-600">¿Estás seguro de que deseas eliminar esta categoría? Si contiene productos, asegúrate de reasignarlos primero. Esta acción es irreversible.</p>
        <x-slot name="footer">
