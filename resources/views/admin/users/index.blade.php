...
<div class="mb-8 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
    <form method="GET" class="w-full flex flex-col md:flex-row gap-4">
        ...
        <div class="w-full md:w-64">
            <select name="role" onchange="this.form.submit()" class="block w-full rounded-xl border-gray-200 py-3.5 text-sm focus:border-amber-800 focus:ring-amber-800 shadow-sm transition-colors font-sans bg-gray-50">
        ...
        <tr class="hover:bg-gray-50/50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 font-sans">
                <div class="flex items-center gap-3">
                    {{-- Avatar minimalista usando iniciales --}}
                    <div class="h-8 w-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 font-bold border border-gray-200 text-xs">
                        {{ strtoupper(substr($user->first_name, 0, 1)) }}{{ strtoupper(substr($user->last_name, 0, 1)) }}
                    </div>
                    {{ $user->first_name }} {{ $user->last_name }}
                </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">{{ $user->email }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">{{ $user->phone ?? '—' }}</td>
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-wrap gap-1">
                    @forelse($user->roles as $role)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200 font-sans">
                            {{ $role->name }}
                        </span>
                    @empty
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center">
                <a href="{{ route('admin.users.edit', $user) }}" class="p-2 text-gray-400 hover:text-amber-900 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
    ...
    @empty
        <tr>
            <td colspan="5" class="px-6 py-12 text-center">
                ...
                <span class="text-sm font-medium text-gray-500 font-sans">No se encontraron usuarios registrados.</span>
            </td>
        </tr>
    @endforelse
</x-admin.table>
