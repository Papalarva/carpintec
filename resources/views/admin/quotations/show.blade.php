            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 border-t-4 border-t-amber-900">
                ...
                <div class="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-sm font-sans">
                    ...
                    @if($quotation->product)
                        <a href="{{ route('admin.products.edit', $quotation->product) }}" target="_blank" class="text-amber-900 hover:text-amber-700 font-bold flex items-center gap-1">
                ...
                @if($customerDocs->count() > 0)
                    <ul class="space-y-2">
                        @foreach($customerDocs as $doc)
                            <li>
                                <a href="{{ route('admin.quotations.download-file', [$quotation, $doc->id]) }}" target="_blank" class="flex items-center p-2 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <form method="POST" action="{{ route('admin.quotations.update-status', $quotation) }}" enctype="multipart/form-data" class="space-y-5" @submit="isSubmitting = true">
                    ...
                    <div x-data="{ files: [] }">
                        ...
                        <template x-if="files.length > 0">
                            <ul class="mt-2 text-xs text-amber-800 space-y-1 font-sans bg-white p-3 rounded-lg border border-amber-100">
                                <template x-for="file in files" :key="file.name"><li x-text="file.name" class="truncate"></li></template>
                            </ul>
                        </template>
                    </div>

                    <button type="submit" :disabled="isSubmitting" :class="{'opacity-75 cursor-not-allowed': isSubmitting}" class="w-full bg-amber-900 text-white hover:bg-amber-800 uppercase tracking-widest text-xs font-bold rounded-xl py-4 transition-colors shadow-md mt-4 flex items-center justify-center">
        ...
        <div class="lg:col-span-7 flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden h-[800px]">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/50" id="admin-chat-container">
                ...
                @if($quotation->response)
                    <div class="flex justify-end">
                        <div class="bg-gray-100 border border-gray-200 rounded-2xl rounded-tr-sm p-4 max-w-[85%] shadow-sm">
                            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Nota Histórica</p>
                ...
                @forelse($quotation->messages ?? [] as $msg)
                            ...
                            <div class="flex justify-start">
                                <div class="bg-white border border-gray-200 rounded-2xl rounded-tl-sm p-4 max-w-[85%] shadow-sm">
                                    <div class="flex justify-between items-baseline mb-1 gap-4">
                                        <p class="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">Cliente</p>
                                        <span class="text-[9px] text-gray-400">{{ $msg->created_at->format('d/m H:i') }}</span>
                                    </div>
                                    @if($msg->hasMedia('chat_images'))
                                        <div class="mb-2 mt-1 cursor-zoom-in overflow-hidden rounded-lg border border-gray-100" @click="lightboxImage = '{{ $msg->getFirstMediaUrl('chat_images') }}'; lightboxOpen = true">
                        ...
                        @else
                            <div class="flex justify-end">
                                <div class="bg-gray-800 text-white rounded-2xl rounded-tr-sm p-4 max-w-[85%] shadow-sm">
                                    <div class="flex justify-between items-baseline mb-1 gap-4">
                                        <p class="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Taller (Carpintec)</p>
                                        <span class="text-[9px] text-gray-400">{{ $msg->created_at->format('d/m H:i') }}</span>
                                    </div>
                                    @if($msg->hasMedia('chat_images'))
                        ...
                        @endif
                @empty
                    <div class="text-center text-gray-400 text-sm py-4 italic mt-10">
            ...
            <div class="p-4 border-t border-gray-100 bg-white shrink-0" x-data="{ imagePreview: null, isSubmitting: false }">
                <form action="{{ route('admin.quotations.message', $quotation) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-2" @submit="isSubmitting = true">
                    ...
                    <div x-show="imagePreview" style="display: none;" class="relative inline-block w-24 h-24 mb-2" x-cloak>
                        ...
                        <button type="button" @click="imagePreview = null; $refs.chatImage.value = null;" class="absolute -top-2 -right-2 bg-white text-red-500 hover:text-red-700 rounded-full p-1 shadow-md border border-gray-100 focus:outline-none transition-colors">
                    ...
                    <div class="flex items-end gap-3">
                        ...
                        <button type="submit" :disabled="isSubmitting" :class="{'opacity-50 cursor-not-allowed': isSubmitting}" class="bg-gray-900 text-white hover:bg-gray-800 rounded-xl p-3.5 h-[50px] w-[50px] flex items-center justify-center transition-colors shadow-sm shrink-0">
