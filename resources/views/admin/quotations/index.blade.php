...
<x-admin.table :headers="['Asunto', 'Cliente', 'Producto', 'Precio est.', 'Estado', 'Acciones']" :rows="$quotations">
    @forelse($quotations as $quotation)
        <tr>
            ...
            <td class="px-6 py-4">{{ $quotation->customer->user->first_name }} {{ $quotation->customer->user->last_name }}</td>
            ...
            <td class="px-6 py-4 text-sm">
                <a href="{{ route('admin.quotations.show', $quotation) }}" class="text-indigo-600 hover:text-indigo-900">Ver</a>
            </td>
        </tr>
    @empty
        <tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No hay cotizaciones.</td></tr>
    @endforelse
</x-admin.table>
@endsection