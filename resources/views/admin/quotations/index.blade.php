    ...
    <x-admin.table :headers="[
        ...
        @forelse($quotations as $quotation)
            <tr class="hover:bg-gray-50/50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">{{ $quotation->created_at->format('d/m/Y') }}</td>
                <td class="px-6 py-4 text-sm font-medium text-gray-900 font-sans">{{ Str::limit($quotation->subject, 40) }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-sans">{{ $quotation->product?->name ?? 'Proyecto a medida' }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center">
                    <a href="{{ route('admin.quotations.show', $quotation) }}" class="p-2 text-gray-400 hover:text-amber-900 hover:bg-amber-50 rounded-lg transition-colors" title="Ver Detalles">
        ...
        @empty
            <tr>
                <td colspan="7" class="px-6 py-12 text-center">
                    <svg class="mx-auto h-12 w-12 text-gray-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    <span class="text-sm font-medium text-gray-500 font-sans">No se encontraron cotizaciones.</span>
                </td>
            </tr>
        @endforelse
    </x-admin.table>
