...
<div x-data="{ actionUrl: '' }">
    ...
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <form method="GET" class="flex flex-1 w-full md:w-auto items-center gap-4">
            <div class="relative w-full md:w-96">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Buscar por código o descuento..."
                       class="block w-full pl-11 pr-4 rounded-xl border-gray-200 py-3.5 text-sm focus:border-amber-800 focus:ring-amber-800 shadow-sm transition-colors font-sans bg-gray-50">
            </div>
        ...
        <a href="{{ route('admin.coupons.create') }}" class="w-full md:w-auto text-center bg-amber-900 text-white hover:bg-amber-800 uppercase tracking-widest text-sm font-bold rounded-xl px-8 py-4 transition-colors duration-200 shadow-sm font-sans whitespace-nowrap">
            Nuevo Cupón
        </a>
    </div>
            ...
            @forelse($coupons as $coupon)
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                        {{-- Diseño estilo Ticket --}}
                        <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-bold font-mono tracking-widest text-amber-900 bg-amber-50 border-2 border-dashed border-amber-200">
                            {{ $coupon->code }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-sans">
                        ...
                        <span class="{{ $isMaxed ? 'text-red-700 font-bold' : 'text-gray-900 font-medium' }}">
                            {{ $coupon->used_count }}
                        </span> 
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-sans">
                        @if($coupon->expires_at)
                            @if($coupon->expires_at->isPast())
                                <span class="inline-flex items-center gap-1 text-red-700 font-bold bg-red-50 px-2.5 py-1 rounded-md">
                            ...
                            @else
                                <span class="text-gray-600">{{ $coupon->expires_at->format('d/m/Y H:i') }}</span>
                            @endif
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm flex items-center gap-2">
                        <a href="{{ route('admin.coupons.edit', $coupon) }}" class="p-2 text-gray-400 hover:text-amber-900 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
                        ...
                        <button @click="actionUrl = '{{ route('admin.coupons.destroy', $coupon) }}'; $dispatch('open-modal', 'delete-modal')" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-16 text-center">
                        ...
                        <h3 class="text-sm font-bold text-gray-900 font-sans uppercase tracking-widest">Sin Cupones</h3>
                        <p class="mt-2 text-sm text-gray-500 font-sans">No hay códigos promocionales generados actualmente.</p>
                    </td>
                </tr>
            @endforelse
        </x-admin.table>
    ...
    <x-admin.modal id="delete-modal" title="Eliminar Cupón">
        <p class="text-gray-600 font-sans leading-relaxed">¿Estás seguro de que deseas eliminar este cupón? Los clientes ya no podrán utilizar este código para obtener descuentos en su carrito de compras.</p>
        <x-slot name="footer">
