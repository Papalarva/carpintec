<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8" x-data="cartPage({{ $subtotal ?? 0 }}, {{ $items->sum('quantity') ?? 0 }})">
            ...
            <ul class="divide-y divide-gray-100" role="list">
                @forelse($items as $item)
                    @php
                        ...
                        $maxStock = 99; // Límite por defecto
                   ...
                   <li class="p-6 flex flex-col sm:flex-row sm:items-center transition-opacity duration-300"
                    ...
                    >
                        <div
                            class="flex-shrink-0 w-24 h-24 sm:w-28 sm:h-28 bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
                        ...
                        <div class="mt-4 sm:mt-0 sm:ml-6 flex-1 flex flex-col sm:flex-row sm:justify-between">
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-gray-900">
                                    <a href="{{ route('catalog.show', $product->slug) }}"
                                        class="hover:text-indigo-600 focus:outline-none focus:underline">
                                        {{ $product->name }}
                                    </a>
                                </h3>
                            ...
                            <div class="mt-4 sm:mt-0 flex flex-col sm:items-end justify-between">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex items-center border border-gray-300 rounded-lg bg-white shadow-sm">
                                        ...
                                        <button @click="increment()" 
                                                ...
                                                class="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-50 hover:text-indigo-600 rounded-r-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-100"
                                    ...
                                    <form action="{{ route('cart.remove', $product->slug) }}" method="POST"
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" aria-label="Eliminar {{ $product->name }} del carrito"
                                ...
                                <p x-show="errorMessage" x-text="errorMessage"
                                    class="text-red-500 text-xs mt-2 font-medium"></p>
                            </div>
                        </div>
                    </li>
                @empty
                    <div class="text-center py-16 px-6">
                        ...
                        <p class="mt-2 text-gray-500 max-w-sm mx-auto">Parece que aún no has decidido qué llevarte.
                            Descubre nuestros muebles de temporada.</p>
                        <a href="{{ route('catalog.index') }}"
                            class="mt-8 inline-flex items-center justify-center bg-indigo-600 text-white px-8 py-3.5 text-sm font-semibold rounded-lg shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all">
        ...
        @if ($items->isNotEmpty())
            <div class="mt-8 bg-gray-50 rounded-xl p-6 sm:p-8 border border-gray-200">
                ...
                <p class="text-sm text-gray-500 border-t border-gray-200 pt-4">Los impuestos y gastos de envío se
                    calcularán en el siguiente paso.</p>

                <div class="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-4 gap-y-3 sm:gap-y-0">
                    <a href="{{ route('catalog.index') }}"
                        class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3.5 border border-gray-300 text-sm font-medium text-gray-700 bg-white rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors">
                        Seguir comprando
                    </a>
                    <a href="{{ route('checkout.index') }}"
                        class="w-full sm:w-auto inline-flex justify-center items-center px-8 py-3.5 border border-transparent text-sm font-bold text-white bg-indigo-600 rounded-lg shadow-md hover:bg-indigo-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all">
                        Ir al Checkout
    ...
    <script>
        ...
        function cartItem(slug, initialQty, unitPrice, maxStock) {
            return {
                ...
                unitPrice: unitPrice,
                maxStock: maxStock, // Guardamos el límite
                ...
                handleManualInput() {
                    ...
                    // Si el usuario escribe manualmente más del stock, lo bloqueamos visualmente
                    if (this.qty > this.maxStock) {
                        this.qty = this.maxStock;
                        this.errorMessage = `Solo tenemos ${this.maxStock} unidades disponibles.`;
                ...
                async updateCart() {
                    // Si no cambió la cantidad, no hacemos petición
                    ...
                    try {
                        ...
                        // 1. Si falla, extraemos el JSON del backend ANTES de lanzar el error
                        if (!resp.ok) {
                            const errorData = await resp.json().catch(() => ({}));
                            throw new Error(errorData.message || 'Ocurrió un error al actualizar.');
                        }

                        const data = await resp.json();

                        // 2. Éxito: Actualizamos la cantidad válida y variables globales
                        ...
                        // 3. Mostramos el mensaje EXACTO del controlador ("Stock máximo alcanzado...")
                        this.errorMessage = e.message;

                        // 4. Revertimos el input a la última cantidad válida sin recargar la página
                        this.qty = this.previousQty;
                        this.updateLocalSubtotal();

                        // Ocultamos el mensaje de error después de 4 segundos
                        setTimeout(() => {
                            this.errorMessage = '';
                        }, 4000);
                    } finally {
                ...
                increment() {
                    // Validamos contra maxStock en lugar de 99
                    if (this.qty < this.maxStock) {
                        this.qty++;
                        this.updateLocalSubtotal();
                        this.scheduleUpdate();
                    } else {
                        // Feedback inmediato si intenta presionar más
                        this.errorMessage = `Solo tenemos ${this.maxStock} unidades disponibles.`;
                        setTimeout(() => { this.errorMessage = ''; }, 3000);
                    }
                },
