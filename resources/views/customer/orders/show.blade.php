        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex mb-8" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    <li>
                        <a href="{{ route('orders.index') }}" class="text-gray-500 hover:text-amber-800 transition-colors">Mis Compras</a>
                    </li>
                    <li class="flex items-center space-x-2">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                <li class="p-6 flex gap-6">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex justify-between">
                                            <h3 class="font-serif text-lg text-gray-900">{{ $item->product->name ?? 'Producto Personalizado' }}</h3>
                                            <p class="font-medium text-gray-900">${{ number_format($item->unit_price * $item->quantity, 2) }}</p>
                                        </div>
                                        <p class="text-sm text-gray-500 mt-1">Precio unitario: ${{ number_format($item->unit_price, 2) }}</p>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 text-center">
                        ...
                        <span class="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border border-current {{ $order->status_id->color() }}">
                            {{ $order->status_id->label() }}
                        </span>
                        <div class="mt-6 pt-6 border-t border-gray-50">
                            <p class="text-sm text-gray-600 italic">"Gracias por confiar en la artesanía de Carpintec."</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
                        <div class="flex items-center gap-3 mb-4">
                            <svg class="w-5 h-5 text-amber-800" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.129-1.125V11.25c0-4.446-3.542-7.875-7.875-7.875H9"></path></svg>
                        ...
                        <div class="text-sm text-gray-600 space-y-1">
                            ...
                            <p class="pt-2 flex items-center gap-2">
                                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M10.5 1.5H8.25A2.25 2.25 0 006 3.75v16.5a2.25 2.25 0 002.25 2.25h7.5A2.25 2.25 0 0018 20.25V3.75a2.25 2.25 0 00-2.25-2.25H13.5m-3 0V3h3V1.5m-3 0h3m-3 18.75h3"></path></svg>
                    ...
                    <div class="bg-amber-50/50 border border-amber-100 rounded-2xl p-6">
                        <h4 class="text-amber-900 font-bold text-sm uppercase tracking-wider mb-2">¿Necesitas ayuda?</h4>
                        <p class="text-amber-800/80 text-xs leading-relaxed mb-4">Si tienes dudas sobre tu fabricación o tiempos de entrega, contáctanos mencionando tu número de pedido.</p>
                        <a href="{{ route('contact.index') }}" class="text-xs font-bold text-amber-900 underline underline-offset-4">Contactar Soporte</a>
                    </div>
                </div>
            </div>
        </div>
